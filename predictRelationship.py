...
def main():
    ...
    classifier.getRelate("21A","Prader-Willi syndrome")
...
#----------------------------cross Feature
    ...
    def getRelate(self, lnc_name, dis_name):
        if dis_name not in self.dataSet.disease_shift or lnc_name not in self.dataSet.lncRNA_shift:
            print("error: Incorrect disease name or lncRNA name")
            return 'Incorrect disease name or lncRNA name'
        cnt_dis = self.dataSet.disease_shift[dis_name]
        cnt_lnc = self.dataSet.lncRNA_shift[lnc_name]
        dicts = self.dataSet.dataMap
        try:
            if dicts[(cnt_lnc, cnt_dis)] == 1:
                return '100%'
        except KeyError:
            pass
        with tf.Session() as sess:
            self.saver.restore(sess, "./checkPoint/model.ckpt")
            print("Begin predict")
            test_result = sess.run(self.final,
                                   feed_dict={
                                       self.lncRNA_index: [cnt_lnc],
                                       self.disease_index: [cnt_dis],
                                   })
            rr = test_result[0][0] * 100
            rr = round(rr, 2)
            if rr < 1:
                return 'less than 1%'
            else:
